...
def plot_residuals_vs_fitted_grid(
    ...
    *,
    ...
    start_date=None,
    threshold: float = 5.0,
    fitted_low_threshold: float = -3.0,
    lowess_frac: float = 0.30,
    figsize_per_cell=(6.0, 5.0),
    n_cols: int = 2,
):
    """
    Trace un panneau Residuals vs Fitted par modèle.

    Paramètres attendus
    -------------------
    combined_bkt : DataFrame long avec au minimum :
        - ds
        - model_label
        - y_true
        - y_hat
    ...
    try:
        ...
        raise ImportError(
            "statsmodels est requis pour plot_residuals_vs_fitted_grid."
        ) from e

    df = combined_bkt.copy()

    needed = [date_col, model_col, y_true_col, y_hat_col]
    missing = [c for c in needed if c not in df.columns]
    if missing:
        raise ValueError(f"Colonnes manquantes pour residual plot: {missing}")

    df[date_col] = pd.to_datetime(df[date_col], errors="coerce")
    df = df.dropna(subset=[date_col, y_true_col, y_hat_col, model_col]).copy()

    if start_date is not None:
        start_date = pd.Timestamp(start_date)
        df = df.loc[df[date_col] >= start_date].copy()

    if len(df) == 0:
        raise ValueError("Aucune donnée disponible pour tracer residuals vs fitted.")

    df["residual"] = pd.to_numeric(df[y_true_col], errors="coerce") - pd.to_numeric(df[y_hat_col], errors="coerce")
    df = df.dropna(subset=["residual"]).copy()

    models = sorted(df[model_col].astype(str).unique().tolist())
    if len(models) == 0:
        raise ValueError("Aucun modèle disponible pour residuals vs fitted.")

    n_models = len(models)
    n_rows = int(np.ceil(n_models / n_cols))

    fig_w = max(8.0, n_cols * figsize_per_cell[0])
    fig_h = max(5.0, n_rows * figsize_per_cell[1])
    ...
    for ax, m in zip(axes, models):
        ...
        sub = df.loc[df[model_col].astype(str) == str(m)].copy()
        sub = sub.dropna(subset=[y_hat_col, "residual"])

        if len(sub) == 0:
            ax.set_visible(False)
            continue

        x = pd.to_numeric(sub[y_hat_col], errors="coerce")
        ...
        ax.axhline(0, linestyle="--", linewidth=1, color=_DARK_ZERO)
        ax.grid(axis="y", linestyle=_DARK_GRID[1], alpha=_DARK_GRID[0])

        extreme = sub[
            (np.abs(sub["residual"]) > threshold) |
            (pd.to_numeric(sub[y_hat_col], errors="coerce") < fitted_low_threshold)
        ].copy()
        ...
        if len(extreme) > 0:
            ...
            for _, row in extreme.iterrows():
                try:
                    lbl = pd.Timestamp(row[date_col]).strftime("%Y-%m")
                except Exception:
                    lbl = str(row[date_col])

                ax.annotate(
                    lbl,
                    (row[y_hat_col], row["residual"]),
        ...
        ax.set_title(f"{m} — Residuals vs Fitted", fontsize=10, color="#e5e5e5")
        ax.set_xlabel("Fitted values (ŷ)", color="#e5e5e5")
        ax.set_ylabel("Residuals (y − ŷ)", color="#e5e5e5")
