...
def _build_cutoffs_hvblock(
    ...
    *,
    ...
    for i, cutoff in enumerate(ds_all):
        cutoff = pd.Timestamp(cutoff)

        train_end = cutoff - relativedelta(months=int(gap))
        n_train_eff = sum(d <= train_end for d in ds_all)
...
def _build_cutoffs_kfold(
    ...
    *,
    ...
    """
    Approximation simple d'un blocked k-fold temporel :
    on construit les cutoffs admissibles puis on les répartit en n_splits blocs
    contigus, et on garde le dernier cutoff de chaque bloc.
    """
    ds_all = sorted(pd.to_datetime(ts_train["ds"]).dropna().unique())
    ds_all = [pd.Timestamp(x).to_period("M").to_timestamp(how="start") for x in ds_all]
    ...
    for i, cutoff in enumerate(ds_all):
        n_train = i + 1
        has_test = (pd.Timestamp(cutoff) + relativedelta(months=h)) in ds_all
        if has_test and (min_train_n is None or n_train >= int(min_train_n)):
            eligible.append(pd.Timestamp(cutoff))
...
def _eval_cutoffs_with_mlforecast(
    ...
    *,
    ...
    """
    Évalue manuellement une liste de cutoffs avec MLForecast.
    Pour chaque cutoff :
      - fit sur dates <= cutoff - gap
      - construit explicitement la grille future attendue par MLForecast
      - merge les exogènes futures dessus
      - compare les prévisions h-pas avec les vraies valeurs

    Debug activé surtout pour RIDGE et LGBM.
    """
    ...
    if do_debug:
        print(
            ...
            f"| len(ts_train)={len(ts_train)} | n_cutoffs={len(cutoffs)} | h={h} | gap={gap}"
        )
    ...
    for i, cutoff in enumerate(cutoffs, start=1):
        ...
        df_fit = ts_train[ts_train["ds"] <= train_end].copy()
        ...
        # grille complète attendue par MLForecast : 1 ligne par id et date future
        ...
        # merge exogènes futures sur la grille complète
        ...
        # si certaines combinaisons id/date futures n'ont pas d'exogènes, on saute ce cutoff
    ...
    *,
    ...
    for block_idx, (cutoff_start_blk, n_windows_blk) in enumerate(blocks, start=1):
        ...
        params_history.append({
            ...
            "cv_method": cv_method,
            "kfold_n_splits": int(kfold_n_splits),
            "hv_gap": int(hv_gap),
            **best_params,
        })
        tune_history.append({
